...
Теперь для того что бы отображать ошибку необходимо передать саму error в Input. Мы передаем булево значение по этому я передаю два !! для того что бы привести его к булеву значению errors={!!errors.firstName}. И helperText у нас должен быть собственно текстом ошибки.
